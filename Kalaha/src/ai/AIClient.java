...
public class AIClient implements Runnable
{
    ...
    public int getMove(GameState currentBoard)
    {
        ...
        int myMove = getaimove(currentBoard); 
        return myMove;   
    }
    ...
    {	
    	...
    	if(pgs.gameEnded())
        	{
        	if(pgs.getWinner()==maxplay) return 2000;
        	if(pgs.getWinner()==minplay) return -2000;
                else return (pgs.getScore(maxplay)-pgs.getScore(minplay));
        	}
    ...
    public int getaimove(GameState state)
    {
        int max=-4000;
        int ca=0;
        for(int i=1; i<7; i++)
        {
            ...
            {
                    GameState pgs = state.clone();
                    pgs.makeMove(i);
                    int value = minmaxpruning(pgs,10, -3000, 3000);
                    if(value>max)
                    {
                            max=value; 
                            ca=i;
